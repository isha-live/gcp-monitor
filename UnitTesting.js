(function (global, name, Package, helpers, creators) {
    ...
    var ref = (function wrapper(args) {
        var wrapped = function () {
            return Package.apply(Import._import(name), arguments);
        };
        for (var i in args) {
            wrapped[i] = args[i];
        }
    ...
    Import._import =
        ...
        function (uniqueId) {
            ...
            if (typeof ret === 'undefined')
                throw Error('Import error! No library called ' + uniqueId);
            return ret;
        };
    global.Import[name] = (function wrapper(args) {
        var wrapped = function (options) {
            options = options || {};
            options.namespace = options.namespace || !1;
            options.base = options.base || !1;
    ...
    function Package(config) {
        var global = function gimmeGlobal() {
            return this;
        }.apply(null, []);
         ...
         */
        UtgsUnit.DOUBLE_EQUALITY_PREDICATE = function (var1, var2) {
            return var1 == var2;
        };
         ...
         */
        UtgsUnit.TRIPLE_EQUALITY_PREDICATE = function (var1, var2) {
            return var1 === var2;
        };
         ...
         */
        UtgsUnit.TO_STRING_EQUALITY_PREDICATE = function (var1, var2) {
        ...
        UtgsUnit.validateArguments = function (opt, fields) {
            ...
            for (var f = 0; f < fields.length; f++) {
                if (!opt.hasOwnProperty(fields[f])) {
                    throw UtgsUnit.AssertionArgumentError(
                        'Assertions needs property ' + fields[f] + ' in opt argument',
                    );
                }
            }
        ...
        UtgsUnit.assert = function (comment, booleanValue, failureMessage) {
            if (!booleanValue) throw new UtgsUnit.Failure(comment, failureMessage);
        };
  ...
  */
        UtgsUnit.Util.contextManager = function () {
            ...
            if (arguments.length == 1) {
                ...
                return function (body) {
                    ...
                    try {
                        var result = body(ret);
                        if (typeof result !== 'undefined') ret = result;
                    } catch (err) {
        ...
        global.assert = {
            ...
            equals: function (opt) {
                ...
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkEquals(opt.expected, opt.actual),
                    'Expected {0.expected} but was {0.actual}'.__format__(opt),
                );
            },
            ...
            false_: function (opt) {
                ...
                UtgsUnit.assert(
                    opt.comment,
                    opt.actual === false,
                    'Call to false_(boolean) with true',
                );
            },
            ...
            notEqual: function (opt) {
                ...
                UtgsUnit.assert(
                    opt.comment,
                    opt.expected !== opt.actual,
                    'Expected not to be {0.expected}'.__format__(opt),
                );
            },
            ...
            null_: function (opt) {
                ...
                UtgsUnit.assert(
                    ...
                    opt.actual === null,
                    'Expected ' +
                        UtgsUnit.displayStringForValue(null) +
                        ' but was {0.actual}'.__format__(opt),
                );
            },
            ...
            notNull: function (opt) {
                ...
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkNotNull(opt.actual),
                    'Expected not to be ' + UtgsUnit.displayStringForValue(null),
                );
            },
            ...
            undefined_: function (opt) {
                ...
                UtgsUnit.assert(
                    opt.comment,
                    opt.actual === UtgsUnit_UNDEFINED_VALUE,
                    'Expected ' +
                        UtgsUnit.displayStringForValue(UtgsUnit_UNDEFINED_VALUE) +
                        ' but was ' +
                        UtgsUnit.displayStringForValue(opt.actual),
                );
            },
            ...
            notUndefined: function (opt) {
                ...
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkNotUndefined(opt.actual),
                    'Expected not to be ' +
                        UtgsUnit.displayStringForValue(UtgsUnit_UNDEFINED_VALUE),
                );
            },
            ...
            objectEquals: function (opt) {
                UtgsUnit.validateArguments(opt, 'expected actual');
                if (opt.expected === opt.actual) return;
                ...
                if (typeOfVar1 == typeOfVar2) {
                    var primitiveEqualityPredicate =
                        UtgsUnit.PRIMITIVE_EQUALITY_PREDICATES[typeOfVar1];
                    if (primitiveEqualityPredicate) {
                        ...
                        for (var i in opt.expected) {
                            this.objectEquals({
                                comment:
                                    opt.comment + ' found nested ' + typeOfVar1 + '@' + i + '\n',
                ...
                UtgsUnit.assert(
                    ...
                    isEqual,
                    'Expected ' +
                        UtgsUnit.displayStringForValue(opt.expected) +
                        ' but was ' +
                        UtgsUnit.displayStringForValue(opt.actual),
                );
            },
            ...
            arrayEquals: function (opt) {
                UtgsUnit.validateArguments(opt, 'expected actual');
                if (
                    UtgsUnit.trueTypeOf(opt.expected) != 'Array' ||
                    UtgsUnit.trueTypeOf(opt.actual) != 'Array'
                ) {
            ...
            evaluatesToTrue: function (opt) {
                UtgsUnit.validateArguments(opt, 'actual');
                if (!opt.actual) this.fail(opt.comment);
            },
            ...
            hashEquals: function (opt) {
                ...
                for (var key in opt.actual) {
                    this.notUndefined({
                        comment: 'Actual hash had key ' + key + ' that was not expected',
                        actual: opt.expected[key],
                    });
                }
            },
            ...
            contains: function (opt) {
                ...
                this.true_({
                    comment: "Expected '{0.collection}' to contain '{0.value}'".__format__(opt),
            ...
            arrayEqualsIgnoringOrder: function (opt) {
                ...
                var notEqualsMessage =
                    'Expected arrays {0.expected} and {0.actual} to be equal (ignoring order)'.__format__(
                        opt,
                    );
                var notArraysMessage =
                    'Expected arguments {0.expected} and {0.actual} to be arrays'.__format__(opt);
                ...
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkNotUndefined(opt.expected.length),
                    notArraysMessage,
                );
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkNotUndefined(opt.expected.join),
                    notArraysMessage,
                );
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkNotUndefined(opt.actual.length),
                    notArraysMessage,
                );
                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkNotUndefined(opt.actual.join),
                    notArraysMessage,
                );

                UtgsUnit.assert(
                    opt.comment,
                    UtgsUnit.checkEquals(opt.expected.length, opt.actual.length),
                    notEqualsMessage,
                );
            ...
            throws: function (opt, func) {
                ...
                try {
                    ...
                    UtgsUnit.assert(
                        opt.comment,
                        err instanceof opt.expectedError,
                        'Expected thrown error to be of type ' +
                            (opt.expectedError.name || opt.expectedError.toString()),
                    );
                }

                if (!caughtError)
                    throw UtgsUnit.Failure(
                        "No error was thrown, expecting error of type '" + opt.expectedError.name,
                    );
            },
            ...
            doesNotThrow: function (opt, func) {
                ...
                try {
                    ...
                    UtgsUnit.assert(
                        opt.comment,
                        err instanceof opt.unexpectedError,
                        'Did not expect to throw error of type ' + opt.unexpectedError.name,
                    );
                }
            },
            ...
            throwsRangeError: function (comment, func) {
                ...
                return this.throws.call(
                    this,
                    { expectedError: RangeError, comment: comment },
                    func,
                );
            },
            ...
            throwsReferenceError: function (comment, func) {
                ...
                return this.throws.call(
                    this,
                    { comment: comment, expectedError: ReferenceError },
                    func,
                );
            },
        ...
        init: function () {
            ...
            var lookup = function (obj, path) {
                ...
                for (var idx = 0; idx < path.length; idx += 1) {
                    var key = path[idx];
                    if (typeof obj[key] === 'function') obj = obj[key]();
                    else obj = obj[key];
                }
                return obj;
            };
            ...
            if (typeof String.prototype.__format__ === 'undefined') {
                Object.defineProperty(String.prototype, '__format__', {
                    get: function () {
                        var global = function gimmeGlobaal() {
                            return this;
                        }.apply(null, []);
                        var $format = create({});
                        return function () {
